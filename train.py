...
def calc_batch_cre(model, batch, labels, char_dict):
...
def eval(model, loss_fn, val_data, char_dict):
    val_metric = 0
    val_loss = 0
    num = 0
    ...
    for _, (x, y) in val_bar:
        ...
        with torch.no_grad():
            x = torch.Tensor(x[..., np.newaxis]).cuda()
            metric = calc_batch_cre(model, x, y, char_dict)
            val_metric += metric
            y = torch.Tensor(y).long().cuda()
            pred = model(x).permute((1, 0, 2))
            loss = loss_fn(
                pred, y,
                input_lengths=torch.Tensor([pred.shape[0] * 1]).long(),
                target_lengths=torch.Tensor([y.shape[1] * 1]).long()
            )
            val_loss += loss.item()
            val_bar.set_postfix({
...
if __name__ == '__main__':
    ...
    transformations = [
        ...
        lambda img: aug.gaussian_blur(img, std=0.1),
    ...
    if args.cpt is not None:
        ...
        #print(f'\nlen of state dict:\n{len(list(cpt_dict["state_dict"].keys()))}\n\n')
    ...
    if not args.eval:
        ...
        eval(model, loss_fn, val_data, char_dict)
